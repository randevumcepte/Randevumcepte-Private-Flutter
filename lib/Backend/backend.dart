...
Future<Map<String, dynamic>> odaDetayGetir(String odaId) async {
  final response = await http.get(
    Uri.parse('https://app.randevumcepte.com.tr//api/v1/oda_detay/$odaId'),
    headers: {'Content-Type': 'application/json'},
  );
  if (response.statusCode != 200) {
    throw Exception(response.reasonPhrase);
...
Future<void> randevuEkleGuncelle(
  ...
  try {
    ...
    response = await http
        .post(
          Uri.parse(
              'https://app.randevumcepte.com.tr//api/v1/randevuekleguncelle'),
          headers: {'Content-Type': 'application/json'},
          body: jsonEncode(formData),
        )
        .timeout(const Duration(seconds: 60));
...
Future<Map<String, dynamic>> isletmeVerileriGetir(String salonid,bool randevuAlSayfasi,String appbundle,String musteriArama,String hizmetArama,int limit, int offset, {String? randevuId}) async {
  ...
  Map<String, dynamic> formData = {
    ...
    // listeye dahil edilsin (silinmis hizmetli randevular acilabilsin).
...
async {
  Map<String, dynamic> formData = {
    ...
    // Platform: 'ios_voip' (APNs VoIP) | 'android_fcm' (FCM). Sunucu dogru
    // transport ile push gondermek icin token tipini bilmeli.
    'platform' : platform,
...
String arayanBilgiVer(String phone){
  ...
  dynamic musteriAdi = arayanbilgi(phone,'391');
