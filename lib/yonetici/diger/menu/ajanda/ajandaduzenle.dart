...
class AjandaDuzenle extends StatefulWidget {
  ...
  AjandaDuzenle({
    Key? key,
    required this.notdetayi,
    required this.ajandaDataSource,
...
class _AjandaDuzenleState extends State<AjandaDuzenle> {
  ...
  void _initializeData() {
    ...
    _baslikController.text = widget.notdetayi.title != "null" ? widget.notdetayi.title : "";
    _tarihController.text = widget.notdetayi.ajandatarih;
    _saatController.text = widget.notdetayi.ajandasaat;
    _icerikController.text = widget.notdetayi.description;

    // Hatırlatma ayarlarını yükle
    _hatirlatmaAktif = stringToBool(widget.notdetayi.hatirlatma);

    // Dropdown değerini ayarla
    try {
      selectedhatirlatmasaat = sablon.firstWhere(
              (item) => item.id == widget.notdetayi.hatirlatma_saat,
          orElse: () => sablon[1] // Varsayılan: 2 saat
      );
    } catch (e) {
      selectedhatirlatmasaat = sablon[1]; // Varsayılan: 2 saat
    }

    // Saati parse et
    if (widget.notdetayi.ajandasaat.isNotEmpty) {
      try {
        List<String> timeParts = widget.notdetayi.ajandasaat.split(':');
    ...
    print('Başlık: ${_baslikController.text}');
    print('Tarih: ${_tarihController.text}');
    print('Saat: ${_saatController.text}');
    print('Hatırlatma: $_hatirlatmaAktif');
  }
  ...
  Future<void> _saveForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        String hatirlatmaSaati = selectedhatirlatmasaat?.id ?? "2";

        print('KAYDEDİLEN VERİLER:');
        print('ID: ${widget.notdetayi.id}');
        print('Başlık: "$baslik"');
        print('Tarih: "$tarih"');
        print('Saat: "$saat"');
        print('İçerik: "$icerik"');
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('Notu Düzenle', style: TextStyle(color: Colors.black)),
          leading: IconButton(
      ...
      children: [
        ...
        Container(
          ...
          child: TextFormField(
            ...
            decoration: InputDecoration(
              suffixIcon: Icon(Icons.calendar_today, color: Color(0xFF6A1B9A)),
        ...
        Container(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton2<Hatirlatma>(
              ...
              onChanged: (value) {
                setState(() => selectedhatirlatmasaat = value);
              },
              items: sablon.map((item) => DropdownMenuItem(
                value: item,
                child: Padding(
                  padding: EdgeInsets.symmetric(horizontal: 5),
                  child: Text(item.hatirlatma, style: TextStyle(fontSize: 14)),
                ),
              )).toList(),
