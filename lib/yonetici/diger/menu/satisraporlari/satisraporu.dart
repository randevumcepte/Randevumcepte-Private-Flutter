...
class SalesReportsPage extends StatefulWidget {
  ...
  SalesReportsPage({Key? key, required this.isletmebilgi, required this.kullanicirolu}) : super(key: key);
...
class _SalesReportsPageState extends State<SalesReportsPage>
  ...
  bool _isFilterActive = false; // Filtre aktif mi kontrolü
  final DateFormat _dateFormat = DateFormat('dd.MM.yyyy');
  final tlFormat = NumberFormat.currency(locale: 'tr_TR', symbol: '');
  final Map<String, bool> _expandedCards = {};

  // Müşteri popup durumu
  final Map<String, bool> _musteriPopupOpen = {};
  ...
  void initState() {
    ...
    // Tab değişikliğinde filtre durumunu kontrol et
    _tabController.addListener(() {
      _checkFilterStatus();
    });
  }

  // Filtre durumunu kontrol eden fonksiyon
  void _checkFilterStatus() {
    final currentTab = _tabs[_tabController.index];

    // Tarih filtresi varsayılan mı kontrolü
    ...
    // Personel filtresi null mı kontrolü
    final isStaffDefault = _selectedStaff[currentTab] == null;

    setState(() {
      _isFilterActive = !(isDateDefault && isStaffDefault);
    });
  }
  ...
  void initialize() async {
    ...
    hizmetRaporu = await hizmetRaporlari(
      ...
      _selectedStaff['Hizmet']?.id?.toString() ?? '',
    );
    ...
    urunRaporu = await urunRaporlari(
      ...
      _selectedStaff['Ürün']?.id?.toString() ?? '',
    );
    ...
    paketRaporu = await paketRaporlari(
      ...
      _selectedStaff['Paket']?.id?.toString() ?? '',
    );

    personelRaporu = await personelRaporlari(
      widget.isletmebilgi['id'].toString(),
      DateFormat('yyyy-MM-dd').format(_selectedDateRanges['Personel']!.start),
      DateFormat('yyyy-MM-dd').format(_selectedDateRanges['Personel']!.end),
    );
    ...
    setState(() {
      isLoading = false;
      _checkFilterStatus(); // Veriler yüklendikten sonra filtre durumunu kontrol et
    });
  }
  ...
  void _resetFilters() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('$currentTab filtreleri sıfırlandı'),
  ...
  // Tüm tablardaki filtreleri sıfırlama
  void _resetAllFilters() {
    setState(() {
      // Tüm tablar için tarih filtrelerini sıfırla
      ...
      // Tüm tablar için personel filtrelerini sıfırla
      _selectedStaff.forEach((key, value) {
        _selectedStaff[key] = null;
      });
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Tüm filtreler sıfırlandı'),
        backgroundColor: Colors.green,
        duration: Duration(seconds: 2),
      ),
    );
  ...
  Widget _buildQuickDateOption(
      ...
      DateTime? startDate,
      DateTime? endDate,
      Function(DateTimeRange) onTap,
      ) {
    final isSelected = startDate == range.start && endDate == range.end;
          ...
          children: [
              ...
              child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      // Sıfırlama butonu
                      if (_selectedStaff[tab] != null)
                        IconButton(
                          icon: Icon(Icons.refresh, color: Colors.orange),
                          onPressed: () {
                            Navigator.pop(context);
                            setState(() {
                              _selectedStaff[tab] = null;
                              initialize();
                            });
                          },
                          tooltip: 'Personel Seçimini Sıfırla',
                        ),
                      IconButton(
            ...
            ...personeller.map((staff) => ListTile(
              leading: CircleAvatar(
                ...
                child: Text(
                  staff.personel_adi.substring(0, 1).toUpperCase(),
                  style: TextStyle(fontSize: 12, color: Colors.blue[800]),
                ),
              ),
              title: Text(staff.personel_adi),
              trailing: _selectedStaff[tab]?.id == staff.id
                  ? Icon(Icons.check_circle, color: Colors.blue, size: 22)
                  : null,
              onTap: () {
                setState(() {
                  _selectedStaff[tab] = staff;
                  initialize();
                });
                Navigator.pop(context);
              },
  ...
  void _toggleMusteriPopup(String cardId) {
    setState(() {
      _musteriPopupOpen[cardId] = !(_musteriPopupOpen[cardId] ?? false);
    });
  }
  ...
  Widget _buildMusteriCard(BuildContext context, dynamic musteri, int index) {
    ...
    return InkWell(
      onTap: () {
        // Müşteri kartına gitme işlemi
        Navigator.pop(context); // Önce popup'ı kapat

        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => MusteriDetaylari(md: md,isletmebilgi: widget.isletmebilgi, kullanicirolu: widget.kullanicirolu,),
          ),
        );

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${musteri['name']} müşteri kartına yönlendiriliyor...'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Tüm filtreleri sıfırla butonu (filtre aktifse göster)
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              // Aktif filtre varsa sıfırlama butonunu göster
              if (_isFilterActive)
                InkWell(
                  onTap: _resetFilters,
                  borderRadius: BorderRadius.circular(20),
          ...
          if (currentTab != 'Personel') ...[
            ...
            _buildFilterCard(
              ...
              color: Colors.blue,
              isActive: dateRange != _defaultDateRange,
            ),
            ...
            _buildFilterCard(
              ...
              color: Colors.green,
              isActive: _selectedStaff[currentTab] != null,
            ),
          ] else ...[
            ...
            Row(
              children: [
                Container(
                  ...
                  child:
                  Icon(Icons.shopping_basket_outlined, color: Colors.green, size: 24),
                ),
                SizedBox(width: 12),
                Expanded(
                  child: Column(
