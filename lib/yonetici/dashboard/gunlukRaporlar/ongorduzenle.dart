...
class OnGorusmeDuzenleOzet extends StatefulWidget {
  final OnGorusmeDataSource2 ongorusmedatasource;
  final dynamic isletmebilgi;
  final OnGorusme ongorusme;
  const OnGorusmeDuzenleOzet({Key? key,required this.ongorusmedatasource, required this.ongorusme,required this.isletmebilgi}) : super(key: key);

  @override
  _OnGorusmeState createState() => _OnGorusmeState();
}
enum SingingCharacter { kadin, erkek }
class _OnGorusmeState extends State<OnGorusmeDuzenleOzet> {
  ...
  Future<void> initialize() async
  {
    ...
    List<OnGorusmeNedeni> ongorusmenedeniliste = await ongorusmenedeni(seciliisletme!);
    List<Sehir> sehirler = await sehirgetir();
    ...
    setState(() {
      ...
      selectedongorusmeyapan = isletmepersonellerliste.firstWhere((element) => element.id.toString() == widget.ongorusme.personel["id"].toString());
      ...
      selectedongorusmesehir = sehirler.firstWhere((element) => element.id.toString() == widget.ongorusme.il_id.toString());
