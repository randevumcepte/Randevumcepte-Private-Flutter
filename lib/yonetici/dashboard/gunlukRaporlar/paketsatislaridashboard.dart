...
import 'package:randevu_sistem/Models/paketsatislari.dart';
...
class _PaketSatislariDashboardState extends State<PaketSatislariDashboard > {
late PaketSatisDataSource _paketSatisDataSource;
...
void initState() {
  ...
  _controller.addListener(() {
    _paketSatisDataSource.search(_controller.text);
    log(_controller.text);
  });
}
Future<void> initialize() async {
  seciliisletme = await secilisalonid();
  paketsatislarigetir(seciliisletme!,'1','').then((data) {
    setState(() {
...
void _onLoadingStateChanged() {
  ...
  Widget build(BuildContext context) {
    ...
    return _isLoading
      ...
      appBar: AppBar(
        title: Text('Paket Satışları',style: TextStyle(color:Colors.black),),
        leading: IconButton(
            ...
            _paketSatisDataSource.rows.isEmpty
              ...
              child: _paketSatisDataSource != null
                ...
                onCellTap: (DataGridCellTapDetails details) {
                  final tappedRow = _paketSatisDataSource
...
Widget _buildPaginationControls() {
  ...
  final totalPages = (_paketSatisDataSource.totalPages).ceil();
  ...
  return Row(
    ...
    children: [
      IconButton(
        ...
        onPressed: _paketSatisDataSource.currentPage > 1
            ? () {
          setState(  () {

            _paketSatisDataSource.setPage(_paketSatisDataSource.currentPage - 1);
          });
        }
            : null,
      ),
      Text('Sayfa ${_paketSatisDataSource.currentPage} / $totalPages'),
      IconButton(
        icon: Icon(Icons.arrow_forward),
        onPressed: _paketSatisDataSource.currentPage < totalPages
            ? () {
          setState(() {
            _paketSatisDataSource.setPage(_paketSatisDataSource.currentPage + 1);
          });
  ...
  void _showDetailsDialog(BuildContext context,PaketSatisi paket) {
              ...
              children: <Widget>[
                ...
                Form(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      SizedBox(height: 20,),
                      Text(paket.musteri,style: TextStyle(fontWeight: FontWeight.bold),),
                      Divider(color: Colors.black,
                        height: 10,),
                      Row(
                        children: [
                          Text('Paket'),SizedBox(width: 20,),
                          Text(': '),
                          Text(paket.paket)
                        ],
                      ),
                      Row(
                        children: [
                          ...
                          SizedBox(width: 20,),
                          Text(': '),
                          Expanded(child: Text(paket.satan))
                        ],
