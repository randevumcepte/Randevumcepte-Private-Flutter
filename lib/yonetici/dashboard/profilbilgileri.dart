...
class _ProfilBilgileriPageState extends State<ProfilBilgileri> {
  ...
  void _initializeData() async {
    // SharedPreferences'tan güncel kullanıcı verilerini al
    ...
    _adsoyad = TextEditingController(text: _currentUser?.name != 'null' ? _currentUser!.name : '');
    _email = TextEditingController(text: _currentUser?.email != 'null' ? _currentUser!.email : '');
    _telefon = TextEditingController(text: _currentUser?.gsm1 != 'null' ? _currentUser!.gsm1 : '');
    _unvan = TextEditingController(text: _currentUser?.unvan != 'null' ? _currentUser!.unvan : '');
    _cinsiyet = TextEditingController(text: _currentUser?.cinsiyet ?? '');
  ...
  void _updateControllers() {
    _adsoyad.text = _currentUser?.name != 'null' ? _currentUser!.name : '';
    _email.text = _currentUser?.email != 'null' ? _currentUser!.email : '';
    _telefon.text = _currentUser?.gsm1 != 'null' ? _currentUser!.gsm1 : '';
    _unvan.text = _currentUser?.unvan != 'null' ? _currentUser!.unvan : '';
  ...
  Future<void> requestCameraAndGalleryPermission() async {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Profil resmi yüklemek için kamera ve galeri izni gerekmektedir.'),
        actions: <Widget>[
          TextButton(
            child: Text('Vazgeç'),
            onPressed: () {
                                          ...
                                          Container(
                                            ...
                                            child: CircleAvatar(
                                              ...
                                              child: _profileImage == null &&
                                                  (_currentUser?.profil_resim == null ||
                                                      _currentUser!.profil_resim.isEmpty)
                                                  ? Icon(
                                                Icons.person,
                                                size: 50,
                                                color: Colors.white,
                                      ...
                                      ElevatedButton.icon(
                                        onPressed: () {
                                          Navigator.push(
                                            context,
                                            MaterialPageRoute(
                                              builder: (context) => HesapKaldirmaIsletme(kullanici: _currentUser!),
                                            ),
                                          );
                                        },
                                        icon: Icon(
...
Future<void> submitForm(context, String name, String email, String unvan, String gsm1, String password, String cinsiyet) async {
  ...
  if (response.statusCode == 200) {
    ...
    // Sayfayı yenilemek için state management kullanılmalı
    // Bu örnekte, Navigator pop ve push ile sayfayı yeniden yükleyebiliriz
    // Veya daha iyisi, bir state management solution kullanılmalı
