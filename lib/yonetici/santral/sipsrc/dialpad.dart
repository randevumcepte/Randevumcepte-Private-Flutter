...
class _MyDialPadWidget extends State<DialPadWidget> implements SipUaHelperListener {
  ...
  void registrationStateChanged(RegistrationState state) {
    if (!mounted) return; // widget dispose edildiyse setState cagirma (cokme onler)
    setState(() {
  ...
  void callStateChanged(Call call, CallState callState) {
    // Yalnizca GIDEN cagrida CallScreen'e git. Gelen cagri merkezi
    // SipService tarafindan (CallKit + yanitlama) yonetilir.
    if (callState.state == CallStateEnum.CALL_INITIATION &&
        call.direction != "INCOMING") {

      log("aranan " + (call.remote_identity ?? ""));
      Navigator.push(
        context,
  ...
  void santralayarlariniyukle() async {
    ...
    widget.kullanicibilgi.yetkili_olunan_isletmeler.forEach((item) {
      if (item['salon_id'].toString() == seciliisletme.toString()) {
        ...
        dahilisifre = (item["dahili_sifre_webrtc"] ?? '').toString();
      }
    });
    if (mounted) setState(() {});
    // Register'i merkezi SipService yonetir (tek helper, cift register yok).
    await SipService.instance.baslat(dahiliNo: dahili, sifre: dahilisifre);
  }
