...
class _BottomNavigationExampleState extends State<BottomNavigationExample>   {
  ...
  void initState() {
    ...
    _pages = [
      DashBoard(kullanici: widget.kullanici, isletmebilgi: widget.isletmebilgi),
      Takvim(selectedTab: _selectedTab, isletmebilgi: widget.isletmebilgi,kullanici:widget.kullanici,kullanicirolu: kullanicirolu,),
      (widget.uyelikturu > 2 && kullanicirolu < 5) ? CDRRaporlari(isletmebilgi: widget.isletmebilgi,dialPadManager: dialPadManager,scaffoldMessengerKey: widget.scaffoldMessengerKey,kullanici: widget.kullanici) : AjandaNotlar(isletmebilgi: widget.isletmebilgi),
      (widget.uyelikturu > 1 ) ? AdisyonlarPage(kullanicirolu:kullanicirolu,kullanici: widget.kullanici, isletmebilgi: widget.isletmebilgi) : OnGorusmeler(isletmebilgi: widget.isletmebilgi),
  ...
  /*void _initializeSiprix([LogsModel? logsModel]) async {
    ...
    if (Platform.isAndroid) {
      await context.read<DevicesModel>().setForegroundMode(true).catchError(showForegroundHata);
      log("Foreground modu aktif edildi");
    }
  }
  void _dahiliHesabiniEkle(String sipDahili,String sipSifre) {
    log('sip dahili '+sipDahili);
    log('sip şifre' +sipSifre);
    AccountModel account = AccountModel();
    account.sipServer = "34.45.69.65";
    account.sipExtension = sipDahili;
    account.sipPassword = sipSifre;
    account.expireTime=300;
    context.read<AccountsModel>().addAccount(account)
        .catchError(showDahiliHataBar);
            ...
            return Stack(
              ...
              children: [
                BottomNavigationBar(
                  ...
                  items: [
                    BottomNavigationBarItem(icon: Icon(Icons.home_outlined), label: "Özet"),
                    BottomNavigationBarItem(icon: Icon(Icons.calendar_month_outlined), label: "Randevular"),
                    widget.uyelikturu > 2 && kullanicirolu<5 ? BottomNavigationBarItem(icon: Icon(Icons.phone_in_talk), label: "") : BottomNavigationBarItem(icon: Icon(Icons.view_agenda_outlined), label: "Ajanda"),
                    widget.uyelikturu > 1 ? BottomNavigationBarItem(icon: Icon(Icons.copy_all_sharp), label: "Satış Takibi") : BottomNavigationBarItem(icon: Icon(Icons.insert_comment_outlined), label: "Ön Görüşmeler"),
                    BottomNavigationBarItem(icon: Icon(Icons.more_vert), label: "Menü"),
                  ],
                ),
